...
/**
 * This method will be used when the API allows for multiple items per order
 */
handlers._orders.parseItems = (aItems) => {
...
handlers._orders.post = (data, callback) => {
  ...
  // Check that all required fields are filled out
  var productId = typeof (data.payload.productId) == 'number' && data.payload.productId > 0 ? data.payload.productId : false;
  ...
  handlers._tokens.verifyToken(token, email, (tokenIsValid) => {
    if (tokenIsValid) {
      ...
      if (productId && quantity) {
        ...
        // see if there is any previous orders for this user, if not create the orders file and add a first order
        _data.read('orders', email, (err, data) => {

          if (!err && data) {
            // do logic to get latest order id to know what is the next number
            ...
            _data.update('orders', email, data, (err) => {
              if (!err) {
                callback(200);
              } else {
                callback(500, { 'Error': 'Could not push the order into the orders list' });
              }
            });
            ...
            // Store the user
            _data.create('orders', email, order, (err) => {
              if (!err) {
                callback(200);
              } else {
...
handlers._orders.put = (data, callback) => {
  ...
  var email = typeof (data.payload.email) == 'string' && data.payload.email.trim().length > 10 ? data.payload.email.trim() : false;
  var productId = typeof (data.payload.productId) == 'number' && data.payload.productId > 0 ? data.payload.productId : false;
  var orderId = typeof (data.payload.orderId) == 'number' && data.payload.orderId > 0 ? data.payload.orderId : false;
  var quantity = typeof (data.payload.quantity) == 'number' && data.payload.quantity > 0 ? data.payload.quantity : false;
  ...
  handlers._tokens.verifyToken(token, email, (tokenIsValid) => {
    if (tokenIsValid) {
      ...
      if (email && productId && orderId && quantity) {
        ...
        _data.read('orders', email, (err, data) => {
          ...
          if (!err && data) {
            ...
            callback(500, { 'Error': 'There was an error fetching the record to be updated' })
          }
  ...
  if (orderId) {
    ...
    _data.read('orders', email, (err, data) => {
      ...
      if (!err && data) {
        ...
        var newData = data.filter((oItem) => {
          return (oItem.orderId != orderId)
        });

        _data.update('orders', email, newData, (err) => {
          if (!err) {
            callback(200);
          } else {
            callback(500, { 'Error': 'Could not push the order into the orders list' });
